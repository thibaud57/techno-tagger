...
| **Audit d'exactitude** | Cette affirmation est-elle vraie ? | Mené le 2026-08-31, partiel |

Le second a été mené une première fois le 2026-08-31, sur les fiches et rules touchant le packaging, le protocole et l'observabilité. Il reste partiel : ce qui n'a pas été couvert est listé plus bas.
...
| Correction | Portée |
|---|---|
| Hook keyring `hook-keyring.backend.py` inexistant dans le paquet | knowledge et rule keyring, knowledge PyInstaller |
| Module `_rapidfuzz_cpp` inexistant | knowledges PyInstaller et rapidfuzz |
| Coût de `--onedir` décrit avec le comportement de `--onefile` | knowledge PyInstaller, `sys._MEIPASS` compris |
| Hook rapidfuzz, **occurrence oubliée** par la passe précédente | `rules/pyinstaller/build.md`, qui l'affirmait encore |
| `--frozen` prescrit en CI alors qu'il n'échoue pas sur un lock désaccordé | rule et knowledge uv, 4 emplacements, plus les workflows |
| Contrat NDJSON divergent | rule Tauri et knowledge Tauri alignés sur ARCHITECTURE.md |
| Engines Angular cités `^26.0.0` au lieu de `>=26.0.0` | VERSIONS.md, rule app-config |
| Checklist de validation finale non exécutable | PRODUCTION.md, 3 commandes sur 7 échouaient |

> La leçon de la ligne 79 s'est vérifiée deux fois : le hook rapidfuzz survivait dans une rule que la première passe donnait pour corrigée, et `--frozen` était prescrit dans une rule dont les Gotchas disaient déjà l'inverse. **Une correction n'est close qu'après un grep de toutes les formulations du fait, pas de la seule qui a été trouvée.**

**Rapporté mais non vérifié**

16 incohérences restent à instruire, dont 2 traitées le 2026-08-31 (motif de renommage et poussée du rapport, marqués dans BRAINSTORM.md) : 9 entre BRAINSTORM.md et les docs récentes (doublons de fichiers, visibilité du dépôt, un rapport contre deux, signal sonore, 6 plugins Tauri contre 8), 5 relevées dans les ADRs (dont le mode `--onedir` tranché dans un ADR dont le titre ne l'annonce pas, et le pool de pochettes absent de l'ADR qui décide des pools), et 2 contradictions knowledge / rule.

**Non traité**

Les fiches de `docs/knowledges/` non couvertes par la passe du 2026-08-31, soit 15 des 21, et leur recouvrement avec `.claude/rules/`. Le rapport correspondant signale une inversion de responsabilité à corriger d'abord : plusieurs rules portent des pièges que leur fiche ignore. Couper les fiches avant de la corriger reviendrait à couper la moitié la plus pauvre du couple.
