...
- **Taille de page de la recherche Beatport, à choisir ici et non avant.** `/beatport/search` rend aujourd'hui 100 résultats par page, valeur en dur dans techno-scraper (`providers/beatport/client.py`, `_PER_PAGE`), sans paramètre pour en demander moins : mesuré le 2026-09-20, `per_page`, `limit`, `page_size` et `size` sortent tous en `422`, les modèles de paramètres étant en `extra="forbid"`. Beatport lui-même rend 25 par défaut et plafonne à 100.
  ...
  - **Dépendance d'ordre avant implémentation** : la production de techno-scraper tourne en `3.1.4` (dernière version publiée à ce jour, qui ne porte pas ce travail), où ces routes n'ont pas de paramètre `limit` et où les Parameter Models restent en `extra="forbid"` (vérifié sur `main` de techno-scraper). Un `limit` envoyé à cette prod rend `422`. Le sub-project 02 ne pourra envoyer `limit=10` qu'une fois ce contrat releasé côté techno-scraper et le déploiement fait en production ; le numéro de version qui le portera n'est pas déterminé, release-please le fixera au merge. Tant que ce n'est pas fait, ce paramètre reste bloqué côté implémentation.
