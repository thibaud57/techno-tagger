...
## Gotchas
- Depuis la 12.0.0 les backends sont découverts exclusivement par entry points, que PyInstaller n'embarque pas sans hook : `entry_points()` rend une liste vide, keyring bascule sur son backend `fail` et lève `NoKeyringError` (`No recommended backend was available`) dans le binaire, jamais en développement
- Le hook réel est `PyInstaller/hooks/hook-keyring.py`, qui fait à la fois `collect_submodules("keyring.backends")` — c'est **lui** qui résout le backend — et `copy_metadata("keyring")`. Le `.spec` du projet garde la copie des métadonnées en double, volontairement : une régression du hook se verrait sinon uniquement dans le binaire. Les `--hidden-import win32ctypes.pywin32.win32cred` / `win32ctypes.pywin32.pywintypes` restent indispensables, `hook-win32ctypes.core.py` ne couvrant que `win32ctypes.core.*`
- Un fichier `hook-keyring.backend.py` existe réellement, mais à la racine du dépôt GitHub de keyring, pas dans le paquet installé ni dans PyInstaller ou hooks-contrib : aucun entry point ne le référence, PyInstaller ne le découvre jamais. Une recherche rapide peut y mener à tort
