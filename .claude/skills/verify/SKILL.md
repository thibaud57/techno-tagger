...
- Conformité à DESIGN.md sur l'onglet Playlist :
  ...
  - aucun défilement de page (`document.scrollingElement` et `main`), la table remplit la hauteur restante, à 1280 × 800 puis au plancher 1024 × 700 par `Emulation.setDeviceMetricsOverride`. Relever les lignes visibles du rapport : le 2026-09-17, résumé replié, 16 lignes à 1280 × 800 et 13 au plancher (2 et aucune avec le formulaire affiché) ; le conteneur de la table descend jusqu'à la ligne « Extraction terminée », vide ou rempli
...
- Un survol calculé sur la position théorique d'une ligne du scroll virtuel peut tomber hors de sa zone visible, sur `main`. Viser un tag dont `document.elementFromPoint` rend bien le badge
