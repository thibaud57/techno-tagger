...
- Onglet Playlist, états vides : avant toute extraction, bloc encadré « Aucune extraction lancée » ; pendant l'extraction, lignes squelette dans la table du rapport (l'extraction de la démo dure 0,2s : les compter par un `MutationObserver`) ; « Passer au tagging » en `primary` outlined (`p-button-outlined` sans `p-button-secondary`)
