...
Agrégats : `just lint`, `just typecheck`, `just test`. Build : `just build-ui`, `just build-sidecar`, `just build`.
...
- **`just test` échoue tant qu'aucun test n'existe.** C'est l'état attendu du squelette, pas une régression : la couverture est à 0 % et le seuil est à 80 %.
